...
export function MediaPicker({
  ...
  onSelectMany?: (assets: Array<MediaAsset>) => void;
  multiple?: boolean;
  ...
  const [picked, setPicked] = useState<Array<MediaAsset>>([]);
  ...
  // In multi-select mode a click toggles membership instead of committing,
  // so the batch is only handed to the caller when it confirms.
  const pick = (asset: MediaAsset) => {
    ...
    setPicked((current) =>
      current.some((item) => item.key === asset.key)
  ...
  const importUrl = async () => {
    ...
    try {
      ...
      pick({ ...media, postCount: 0, isCover: false });
    } catch {
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 min-h-0">
          {isPending ? (
            ...
            <MediaGrid
              ...
              showMeta={false}
              selectedKeys={
                multiple ? new Set(picked.map((item) => item.key)) : undefined
              }
            />
          )}
        </div>
        {multiple ? (
          <div className="px-5 pb-5 flex items-center justify-between gap-3 border-t border-(--fuwari-input-border) pt-4">
            <span className="text-xs fuwari-text-50">
              {m.editor_insert_picked({ count: picked.length })}
            </span>
            <button
              type="button"
              disabled={picked.length === 0 || !onSelectMany}
              onClick={() => onSelectMany?.(picked)}
              className="fuwari-btn-primary rounded-xl h-10 px-4 text-sm font-medium disabled:opacity-50"
            >

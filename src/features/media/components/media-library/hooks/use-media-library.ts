...
export function useMediaLibrary() {
  ...
  const { unused, view, kind } = useSearch({ from: "/admin/media/" });
  ...
  const setMimeType = (val: MediaKind) => {
    navigate({
      search: (prev) => ({ ...prev, kind: val === "all" ? undefined : val }),
    });
  };
  ...
  const {
    ...
    ...mediaInfiniteQueryOptions("", unused ?? false, kind ?? "all"),
  });
